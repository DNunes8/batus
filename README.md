...
- **The icons are static PNGs on purpose.** `src/app/icon.png` and
  `apple-icon.png` used to be generated by `next/og` (`ImageResponse`), which
  drags a 1.4 MB WASM font renderer into the Worker bundle and pushed it to 92%
  of Cloudflare's 3 MiB free-plan ceiling. The PNGs are the exact bytes that
  generator produced. If the real logo ever arrives, replace the files — don't
  go back to generating them.
